...
def render_html(payload: dict[str, Any]) -> str:
    ...
    for category in sorted(summary, key=category_rank):
        ...
        lb_rows = [
            "<div class='lb-head'><span>Tool</span><span></span>"
            "<span class='h-r'>Coverage</span><span>Evidence</span>"
            "<span class='h-r'>Speed</span></div>"
        ]
        for tool_label, group in ordered:
            ...
            lb_rows.append(
                "<div class='lb-row'>"
                f"<span class='lb-name'>{html.escape(tool_label)}</span>"
                "<div class='bar-track'>"
                f"<div class='bar-fill' style='width:{score_width}%'></div>"
                "</div>"
                f"<strong class='lb-facet-n'>{pct(avg_score)}</strong>"
                f"<span class='{evidence_class(avg_score)} lb-ev'>{evidence_label(avg_score)}</span>"
                f"<span class='lb-hall'>{whole(avg_elapsed)} ms</span>"
                "</div>"
            )
        ...
        category_sections.append(
            ...
            "<div class='chart lb lb-tool' aria-label='Tool leaderboard'>"
            + "\n".join(lb_rows)
            + "</div>"
            "</section>"
        )
    ...
    head = (tpl / "head.html").read_text(encoding="utf-8")
    models = (tpl / "models.html").read_text(encoding="utf-8")
    footer = (tpl / "footer.html").read_text(encoding="utf-8")
    return head + models + category_html + footer
