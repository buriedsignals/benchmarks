...
- 2026-07-03 — post-approval addendum — Tom approved: (a) paginated multi-page browser case — added wikidata_zurich_paginated_search (live-validated: page 1 exposes only the offset=20 pager link; the 'Results 41 – 60' count line exists only on page 3; weight-3 regex probe). Instant discrimination: dev-browser 1.0, browser-use CLI 1.0, Playwright script 0.4 (click chain breaks between hops — same selectors dev-browser passed), Stagehand 0.4 (agent stops paginating). Browser category now has real spread. (b) LlamaParse unesco at 600s: 1.0 — PDF matrix now 100% complete; the earlier 0.0 confirmed as pure invocation artifact. Final: Browser 89% (5 cases, honest spread), PDF 94%, Scraping 70%. Publishing per Tom's instruction.
